...
function buttonSizeMixin(size: ButtonSize) {
...
function buttonTypeMixin(type: ButtonType) {
...
function buttonRippleMixin(type: ButtonType) {
  const bt = buttonType[type]
  return [
...
const rippleStyle = Object.keys(buttonType).map((type) => {
  return buttonRippleMixin(type as ButtonType)
})
...
const buttonStyle = cB('button', {
  ...
  ...Object.keys(buttonType).map(type => buttonTypeMixin(type as ButtonType)),
])
